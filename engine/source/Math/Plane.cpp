...
float Plane::intersects(const Plane& plane) const
{
    ...
    Vec3 point(plane._normal.x * -plane._distance, plane._normal.y * -plane._distance, plane._normal.z * -plane._distance);
...
Vec3 Plane::getPointOfIntersectionToRay(Vec3 V1, Vec3 V2) {
    
    Vec3 Diff = V1 - V2;
    float Denominator = _normal.x * Diff.x + _normal.y * Diff.y + _normal.z * Diff.z;
    if(Denominator == 0.0f)
    {
        return (V1 + V2) * 0.5f;
    }
    
    float u = (_normal.x * V1.x + _normal.y * V1.y + _normal.z * V1.z + _distance) / Denominator;
    
    return (V1 + Vec3(V2 - V1) * u);
